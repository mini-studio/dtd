 ...
 */
public class MiniCustomGridView extends GridView{
    ...
    private List<Integer> mDefaultGirdList = new ArrayList<Integer>();
    ...
    public void setDefalutGrid(List<Integer> defaultGirdList) {
        this.mDefaultGirdList = defaultGirdList;
    }
    ...
    public MiniCustomGridView(Context context) {
        super(context);
        init(context);
    }

    public MiniCustomGridView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public MiniCustomGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void startWobbleAnimation() {
        ...
        for (int i = 0; i < getCount(); i++) {
            View v = getChildAt(i);
            if (v != null) {
                v = v.findViewById(R.id.item_layout);

                if (getFirstVisiblePosition() < mDefaultGirdList.size() && i < mDefaultGirdList.size()) {
                    v.setTag(R.id.dgv_wobble_tag, true);
                } else if (getLastVisiblePosition() == getCount() - 1 && i == getChildCount() - 1) {
                    v.setTag(R.id.dgv_wobble_tag, true);
                }
    ...
    public void init(Context context) {
        ...
        mOverlapIfSwitchStraightLine = getResources()
                .getDimensionPixelSize(R.dimen.dgv_overlap_if_switch_straight_line);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    private void animateWobble(View v) {
        if (Boolean.TRUE == v.getTag(R.id.dgv_wobble_tag)) {
    ...
    private MiniCustomGridAdapterInterface getAdapterInterface() {
        return ((MiniCustomGridAdapterInterface) getAdapter());
    }
    ...
    private void startDragAtPosition(int position) {
        if (position < mDefaultGirdList.size()) {
            return;
        }
        mTotalOffsetY = 0;
        mTotalOffsetX = 0;
    ...
    private void handleCellSwitch() {
        ...
        for (Long id : idList) {
            ...
            if (view != null) {
                ...
                if ((aboveRight(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal < view.getBottom()
                        && deltaXTotal > view.getLeft() || aboveLeft(targetColumnRowPair, mobileColumnRowPair)
                        && deltaYTotal < view.getBottom() && deltaXTotal < view.getRight()
                        || belowRight(targetColumnRowPair, mobileColumnRowPair) && deltaYTotal > view.getTop()
                        && deltaXTotal > view.getLeft() || belowLeft(targetColumnRowPair, mobileColumnRowPair)
                        ...
                        && deltaXTotal > view.getLeft() + mOverlapIfSwitchStraightLine || left(targetColumnRowPair,
                        mobileColumnRowPair) && deltaXTotal < view.getRight() - mOverlapIfSwitchStraightLine)) {
                    float xDiff = Math.abs(MiniCustomGridUtils.getViewX(view) - MiniCustomGridUtils.getViewX(mobileView));
                    float yDiff = Math.abs(MiniCustomGridUtils.getViewY(view) - MiniCustomGridUtils.getViewY(mobileView));
        ...
        if (targetView != null) {
            ...
            if (targetPosition < mDefaultGirdList.size() || targetPosition == getCount() - 1) {
                return;
            }
