...
/**
 * Created by gassion on 7/17/15.
 */
public class MiniSpannableUtils {

    public static MiniSpannableUtils mInstance;
    ...
    private MiniSpannableUtils() {
    }

    public static MiniSpannableUtils getInstance() {
        if (mInstance == null) {
            mInstance = new MiniSpannableUtils();
        }
        return mInstance;
    }
    ...
    private void addLinks() {
        // fix by ygzhang 兼容Android5.0以上系统
        // Linkify.addLinks(mTextView, WEB_PATTERN, mTextView.getText().toString());
        // Linkify.addLinks(mTextView, EMAIL_PATTERN, mTextView.getText().toString());
    ...
    private void openUrl(String url) {
        MNWebViewActivity.open(mContext, url);
    }


    private void dialNum(final String num) {
        if (num != null && num.length() > 0) {
            MiniSystemHelper.call(num, mContext);
        }
    }
